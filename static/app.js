...
// 更新停止按钮为重新搜索状态
function updateStopButtonToRestart() {
        ...
        .then(response => {
            ...
            function processData() {
                reader.read().then(({ done, value }) => {
                    if (done) {
                        console.log('流数据读取完成');
                        // 只做清理工作，状态更新已在接收end/stopped信号时完成
                        return;
                    }
...
function addLog(message, level = 'info') {
    ...
    logEntry.className = 'log-entry flex items-start space-x-3 text-sm border-b border-slate-50 pb-1 mb-1 last:border-0';
    
    // 根据日志级别设置颜色 (适配浅色背景)
    let colorClass = 'text-slate-600'; // 默认信息用 Slate 深灰色
    
    if (level === 'warning') colorClass = 'text-amber-600'; // 警告用深琥珀色
    if (level === 'error' || level === 'danger') colorClass = 'text-red-600'; // 错误用深红色
    if (level === 'success') colorClass = 'text-emerald-600 font-medium'; // 成功用翠绿色
    
    // 如果是 info 级别，保持 slate 色，但可以加深一点
    if (level === 'info') colorClass = 'text-slate-700';

    logEntry.innerHTML = `
        <span class="text-slate-400 text-xs mt-0.5 font-mono shrink-0">${timestamp}</span>
...
// 清空日志 (浅色系适配版)
function clearLogs() {
    logConsole.innerHTML = '<div class="text-slate-400 text-center pt-8">等待搜索开始...</div>';
}
